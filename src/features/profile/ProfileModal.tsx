...
import { readLocal } from '@/shared/lib/storage';
import { CHECKINS_KEY, FAVORITES_KEY } from '@/features/atlas/atlas.data';
...
export const ProfileModal: React.FC<ProfileProps> = ({
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const data = await apiRequest<AtlasEntry[]>('/atlas/favorites');
        if (Array.isArray(data)) setFavorites(data);
      } catch {
        // ignore
      }
      try {
        const data = await apiRequest<AtlasEntry[]>('/atlas/checkins');
      ...
      <div className="bg-paper w-full max-w-md h-[85vh] sm:h-[600px] rounded-t-2xl sm:rounded-xl shadow-2xl overflow-hidden pointer-events-auto flex flex-col relative animate-slide-up bg-texture border border-stone-200">
        ...
        <div className="bg-ink text-paper p-6 relative overflow-hidden">
          ...
          <div className="flex items-center gap-4 relative z-10">
            ...
            <div>
              <h2 className="text-xl font-serif font-bold tracking-wide">{user?.name || user?.username || 'Traveler'}</h2>
              ...
              {onExitToShowcase && (
                ...
                >
                  <div className="flex items-center gap-3">
                    <ArrowLeft size={16} className="text-ink group-hover:translate-x-[-2px] transition-transform" />
                    <span className="text-ink font-serif text-sm">返回产品展示页</span>
                  </div>
                  <span className="text-xs text-stone-400">Showcase</span>
                </button>
              )}
              {onShowWelcome && (
                <button 
                ...
                >
                  <div className="flex items-center gap-3">
                    <Sparkles size={16} className="text-stamp group-hover:scale-110 transition-transform" />
                    <span className="text-ink font-serif text-sm">重新查看欢迎页</span>
                  </div>
                  <span className="text-xs text-stone-400">Welcome</span>
                </button>
              )}
