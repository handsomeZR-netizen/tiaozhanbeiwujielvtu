...
import { X, Heart, BookOpen, Share2, Quote } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
      ...
      <div className="relative w-full max-w-sm bg-paper rounded-sm shadow-2xl border border-stone-200 overflow-hidden animate-slide-up bg-texture">
        ...
        <div className="h-32 bg-ink relative overflow-hidden flex flex-col justify-end p-6">
             ...
             <div className="absolute inset-0 opacity-20" style={{ backgroundImage: 'radial-gradient(#8b1a10 1px, transparent 1px)', backgroundSize: '10px 10px' }}></div>
             
             <div className="relative z-10 text-paper">
                 <div className="flex gap-2 mb-2">
                    {data.tags.map(tag => (
                        <span key={tag} className="text-[10px] border border-white/20 bg-white/10 px-2 py-0.5 rounded-sm backdrop-blur-md tracking-wider">{tag}</span>
                    ))}
                 </div>
                 <h2 className="text-3xl font-serif font-bold tracking-wide">{data.title}</h2>
                 <p className="text-xs text-stone-400 font-sans tracking-[0.2em] uppercase mt-1">{data.subtitle}</p>
             </div>
        ...
        <div className="p-6 relative">
             ...
             <div className="mb-6 pt-2">
                <h3 className="text-sm font-bold text-stamp mb-3 flex items-center gap-2 font-serif">
                    <BookOpen size={16}/> 深度解读 (Insight)
                </h3>
                <p className="text-ink font-serif leading-relaxed text-sm text-justify">
                    {data.description}
                </p>
             </div>
             ...
             <div className="bg-stone-50 p-5 border-l-2 border-stone-300 mb-8 relative">
                ...
                <p className="italic text-stone-600 text-xs leading-relaxed font-serif text-justify">
             ...
             <div className="flex gap-3">
                ...
                >
                    ...
                    <span className="text-xs font-bold tracking-wide">{isFavorite ? '已收藏 (Saved)' : '收藏词条 (Save)'}</span>
                </button>
             </div>
        </div>
      </div>
