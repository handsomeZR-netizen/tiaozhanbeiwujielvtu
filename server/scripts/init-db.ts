...
const makeClientFromUrl = (connectionString: string) =>
  new Client({
    connectionString,
...
const ensureSchema = async (dbName?: string) => {
  ...
  const client = connectionString ? makeClientFromUrl(connectionString) : makeClient(dbName ?? 'boundless_lens');
  ...
  try {
    ...
    `);
    await client.query(`CREATE INDEX IF NOT EXISTS sessions_token_idx ON sessions(token);`);
    await client.query(`CREATE INDEX IF NOT EXISTS sessions_user_id_idx ON sessions(user_id);`);
    console.log('Schema ready: sessions');

    await client.query(`CREATE INDEX IF NOT EXISTS diaries_user_id_idx ON diaries(user_id);`);
    await client.query(`CREATE INDEX IF NOT EXISTS posters_user_id_idx ON posters(user_id);`);
    ...
    console.log('Schema ready: community_posts');
    ...
    `);
    await client.query(`CREATE UNIQUE INDEX IF NOT EXISTS atlas_favorites_user_poi_idx ON atlas_favorites(user_id, poi_id);`);
    console.log('Schema ready: atlas_favorites');
    ...
    `);
    await client.query(`CREATE UNIQUE INDEX IF NOT EXISTS atlas_checkins_user_poi_idx ON atlas_checkins(user_id, poi_id);`);
    console.log('Schema ready: atlas_checkins');
    ...
    `);
    console.log('Schema ready: user_profiles');
    ...
    `);
    await client.query(`CREATE INDEX IF NOT EXISTS studio_history_user_id_idx ON studio_history(user_id, created_at DESC);`);
    console.log('Schema ready: studio_history');
  } finally {
    await client.end();
  }
};
